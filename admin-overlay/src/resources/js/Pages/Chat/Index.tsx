...
export default function ChatIndex({
    ...
    async function ask(text: string) {
        ...
        try {
            ...
            const gap = (payload.missing_evidence || []).join(", ");
            setMessages((current) => [
                ...current,
                {
                    role: "agent",
                    text: payload.answer,
                    meta: [payload.status, payload.intent, plan, gap, cites].filter(Boolean).join("  |  "),
                },
            ]);
        } catch {
            setMessages((current) => [
            ...
            <div className="py-8">
                <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[280px_1fr] sm:px-6 lg:px-8">
                    <aside className="space-y-4 bg-white p-5 shadow-xs sm:rounded-lg">
                        <p className="text-xs tracking-wide text-amber-700">サンプル</p>
                        <div>
