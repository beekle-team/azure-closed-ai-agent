...
class RetrievalFacade:
    ...
    def search(self, search_plan: SearchPlan, limit: int | None = None) -> list[RetrievalHit]:
        return self.retrieve(search_plan, limit=limit).hits

    def retrieve(self, search_plan: SearchPlan, limit: int | None = None) -> RetrievalResult:
        ...
        if "skills" in sources:
            ranked.append(self.skills.as_hits(search_plan.question))

        hits = fuse(ranked)[:cap]
