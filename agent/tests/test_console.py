...
AUTH = {"Authorization": "Bearer local-admin"}
...
def test_console_page() -> None:
    ...
    assert "身元" in response.text


def test_knowledge_lists_corpus_and_microsoft() -> None:
    client = TestClient(app)
    items = client.get("/v1/knowledge", headers=AUTH).json()
...
def test_mail_send_keeps_outbox() -> None:
    ...
    assert any("締日" in item["subject"] for item in client.get("/v1/mail/outbox", headers=AUTH).json())
